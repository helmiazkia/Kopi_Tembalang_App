...
<div class="py-6 px-4">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-black text-slate-800 tracking-tight">Menu Option Item</h1>
            <p class="text-slate-500 text-sm">Kelola item pilihan tambahan seperti variasi ukuran, topping, atau level gula.</p>
        </div>
        <button
        ...
        <div class="overflow-x-auto">
            <table class="table table-zebra w-full">
                ...
                <tbody class="text-slate-600">
                    @forelse ($menu_option_items as $index => $item)
                            ...
                            <td>
                                <div class="flex justify-center gap-2">
                                    <button
                                        ...
                                        data-id="{{ $item->id }}"
                                        data-name="{{ $item->name }}"
                                        data-menu_option="{{ $item->menu_option_id }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-12 text-slate-300 italic text-sm">
                                Belum ada option item tersedia.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
{{-- ===================== MODAL: TAMBAH OPTION ITEM ===================== --}}
<dialog id="add_modal" class="modal modal-bottom sm:modal-middle">
    <div class="modal-box max-w-2xl bg-white">
        <form method="POST" action="{{ route('admin.menu_option_items.store') }}">
            @csrf

            <div class="flex justify-between items-center mb-6">
                <h3 class="font-black text-2xl text-slate-800 tracking-tight">Tambah Option Item</h3>
                <button type="button" onclick="add_modal.close()" class="btn btn-sm btn-circle btn-ghost">✕</button>
            </div>

            <div class="form-control mb-4">
                <label class="label"><span class="label-text font-bold text-slate-600">Nama Item</span></label>
                <input type="text" name="name" placeholder="Contoh: Large, Extra Shot, dll."
                    class="input input-bordered w-full" required>
            </div>

            <div class="form-control mb-4">
                <label class="label"><span class="label-text font-bold text-slate-600">Menu Option</span></label>
                <select name="menu_option_id" class="select select-bordered w-full">
            ...
            <div class="form-control mb-6">
                <label class="label"><span class="label-text font-bold text-slate-600">Harga Tambahan (Rp)</span></label>
...
<dialog id="edit_modal" class="modal modal-bottom sm:modal-middle">
    <div class="modal-box max-w-2xl bg-white text-slate-800">
        <form method="POST" id="editForm">
            ...
            <div class="flex justify-between items-center mb-6">
                <h3 class="font-black text-2xl text-slate-800 tracking-tight">Edit Option Item</h3>
            ...
            <div class="form-control mb-4">
                <label class="label"><span class="label-text font-bold text-slate-600">Menu Option</span></label>
                <select id="edit_menu_option" name="menu_option_id" class="select select-bordered w-full">
                    @foreach ($menu_options as $opt)
                        <option value="{{ $opt->id }}">{{ $opt->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-control mb-6">
                <label class="label"><span class="label-text font-bold text-slate-600">Harga Tambahan (Rp)</span></label>
                <input type="number" id="edit_price" name="price" class="input input-bordered w-full" required>
            </div>
...
<dialog id="delete_modal" class="modal">
    <div class="modal-box bg-white max-w-sm text-center">
        ...
        <h3 class="font-black text-xl text-slate-800">Hapus Option Item?</h3>
        <p class="text-slate-500 mt-2">Tindakan ini tidak bisa dibatalkan. Item akan hilang dari daftar.</p>
...
<script>
    function openEditModal(btn) {
        const { id, name, price, menu_option } = btn.dataset;
    ...
    function openDeleteModal(btn) {
        document.getElementById('deleteForm').action = `/admin/menu_option_items/${btn.dataset.id}`;
