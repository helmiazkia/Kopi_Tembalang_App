...
<x-layouts.admin title="Manajemen Menu">
    ...
    <script>
        setTimeout(() => {
            document.querySelector('.toast')?.classList.add('opacity-0', 'transition-opacity', 'duration-500');
            setTimeout(() => document.querySelector('.toast')?.remove(), 500);
        }, 3000)
    </script>
    @endif
    ...
    <div class="py-6 px-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
           ...
           <button class="btn border-none bg-[#D4E971] hover:bg-black hover:text-[#D4E971] text-black shadow-xl shadow-[#D4E971]/30 px-8 rounded-2xl font-black transition-all duration-500 group"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" class="mr-2 group-hover:rotate-90 transition-transform duration-500"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
                Tambah Menu Baru
            </button>
        </div>
                ...
                <table class="table table-zebra w-full">
                    ...
                    <tbody class="text-slate-600">
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td>
                                <span class="badge badge-ghost border-slate-200 text-slate-500 font-medium">{{ $menu->category->name }}</span>
                            </td>
                            <td>
                                <div class="flex flex-wrap gap-1">
                                    @forelse($menu->options as $opt)
                                    <span class="text-[10px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider border border-slate-200">{{ $opt->name }}</span>
                                    @empty
                            ...
                            <td class="font-black text-slate-800">
                                <span class="text-[10px] font-normal opacity-40">Rp</span> {{ number_format($menu->price, 0, ',', '.') }}
                            </td>
                            <td>
        ...
        <div class="modal-box max-w-2xl bg-white">
            <form method="POST" enctype="multipart/form-data" action="{{ route('admin.menus.store') }}">
                ...
                <div class="mt-6">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        ...
                        <label class="flex items-center gap-3 border border-slate-100 p-3 rounded-xl cursor-pointer hover:bg-slate-50 transition-colors has-[:checked]:bg-[#D4E971]/10 has-[:checked]:border-[#D4E971]">
                            <input type="checkbox" name="menu_option_ids[]" value="{{ $opt->id }}" class="checkbox checkbox-sm checkbox-primary rounded-md border-slate-300">
                ...
                <div class="modal-action mt-8">
                    ...
                    <button class="btn border-none bg-[#D4E971] hover:bg-[#c2d75f] text-black px-10 rounded-xl font-bold">Simpan Produk</button>
                </div>
            </form>
        </div>
    </dialog>

    {{-- EDIT MODAL (Sama dengan Add Modal tapi dengan ID edit) --}}
        ...
        <div class="modal-box max-w-2xl bg-white text-slate-800">
            <form method="POST" enctype="multipart/form-data" id="editForm">
                ...
                <div class="mt-6 text-slate-800">
                    <label class="label"><span class="label-text font-bold text-slate-600 tracking-tight">Opsi Menu</span></label>
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        @foreach($options as $opt)
                        <label class="flex items-center gap-3 border border-slate-100 p-3 rounded-xl cursor-pointer hover:bg-slate-50 transition-colors has-[:checked]:bg-[#D4E971]/10 has-[:checked]:border-[#D4E971]">
                            <input type="checkbox" name="menu_option_ids[]" value="{{ $opt->id }}" class="edit-option checkbox checkbox-sm checkbox-primary rounded-md">
                            <span class="text-sm font-medium text-slate-700">{{ $opt->name }}</span>
                        </label>
                        @endforeach
                    </div>
                ...
                <div class="modal-action mt-8 text-slate-800">
                    <button type="button" class="btn btn-ghost px-8" onclick="edit_modal.close()">Batal</button>
                    <button class="btn border-none bg-[#D4E971] hover:bg-[#c2d75f] text-black px-10 rounded-xl font-bold shadow-lg shadow-[#D4E971]/20">Perbarui Menu</button>
                </div>
            </form>
        </div>
    </dialog>
    ...
    <dialog id="delete_modal" class="modal">
        <div class="modal-box bg-white max-w-sm text-center">
            ...
            <form method="POST" id="deleteForm">
                @csrf
                @method('DELETE')
                <div class="flex flex-col gap-2 mt-6 text-slate-800">
                    <button class="btn btn-error text-white font-bold rounded-xl border-none">Ya, Hapus Sekarang</button>
    ...
    <script>
        function openEditModal(btn) {
            ...
            document.querySelectorAll('.edit-option').forEach(cb => {
                cb.checked = options.includes(parseInt(cb.value));
            });

            document.getElementById('editForm').action = `/admin/menus/${data.id}`;
