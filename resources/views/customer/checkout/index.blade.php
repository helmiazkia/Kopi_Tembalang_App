<x-layouts.cashier title="Kasir POS">
    ...
    <style>
        ...
        .menu-card:hover:not(.out-of-stock) .thumb img {
        ...
        <div class="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl border border-slate-100 mx-4 text-center">
            ...
            <div id="pay-step-2" class="hidden space-y-3">
                ...
                <button onclick="submitPayment('qris')"
                    ...
                    <span class="text-xl">📱</span>
                    <span class="font-black uppercase text-xs text-slate-700">Pembayaran Online</span>
                </button>
                <button onclick="cancelPayment()"
...
════════════════════════════════════════════════════════ --}}
    <script>
        ...
        function buildOptionsHTML(options) {
            return options.map(opt => {
                ...
                const badge = isSelect ?
                    '<span class="bg-red-100 text-red-500 text-[8px] font-black px-2 py-0.5 rounded-lg uppercase">⚠ Wajib Pilih 1</span>' :
                    '<span class="bg-blue-100 text-blue-600 text-[8px] font-black px-2 py-0.5 rounded-lg uppercase">Opsional</span>';
        ...
        function stepOption(itemId, delta, price, name, groupId) {
            if (!currentSelected[itemId]) currentSelected[itemId] = {
                id: itemId,
                qty: 0,
                price,
                name
        ...
        // ── Cart ───────────────────────────────────────────────────────────
        function confirmAdd() {
            // Validasi: semua option tipe 'select' wajib dipilih
            const missing = currentItem.options
                .filter(opt => opt.type === 'select')
                .filter(opt => !opt.items.some(i => currentSelected[i.id]));

            if (missing.length > 0) {
                const names = missing.map(o => `"${o.name}"`).join(', ');
                alert(`Pilih opsi wajib terlebih dahulu: ${names}`);
                return;
            }
